...
// Update the display of selected labels
...
function parseJiraIssueData(jiraIssue) {
    ...
    const userSetFields = ['summary', 'description', 'issuetype']; // Fields that user directly set
...
// Utility function to get proxy API endpoint URL
function getProxyApiUrl(endpoint) {
    if (AppState.useProxy) {
        return `${AppState.proxyUrl}/${endpoint}`;
    } else {
        return getJiraApiUrl(AppState.jiraUrl, endpoint);
    }
}

// Utility function to create fetch headers
